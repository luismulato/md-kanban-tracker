...
import { readRunningTimerTask } from '../../automation/timelog';
...
describe('applyBoardAutomation', () => {
  it('starts the timer automatically when a task enters an empty WIP', () => {
    ...
    const result = applyBoardAutomation({
      ...
      companionDir: tmpDir,
      boardBaseName: 'board',
      previousWipTitle: null,
      justMovedTitle: 'Tarea A',
      now: new Date(2026, 7, 22, 10, 0),
    });

    expect(result.newWipTitle).toBe('Tarea A');
  ...
  it('switches focus in one step: stops the outgoing task and starts the incoming one', () => {
    ...
    const result = applyBoardAutomation({
      ...
      companionDir: tmpDir,
      boardBaseName: 'board',
      previousWipTitle: 'Tarea A',
      justMovedTitle: 'Tarea B',
      now: new Date(2026, 7, 22, 9, 30),
    });

    expect(result.newWipTitle).toBe('Tarea B');
    expect(readRunningTimerTask(tmpDir)).toBe('Tarea B');

    const log = fs.readFileSync(path.join(tmpDir, 'board.timelog.md'), 'utf-8');
    expect(log).toContain('"Tarea A" (auto, cambio de foco en WIP)');
  });

  it('does not touch an ad-hoc timer for a task that is not the WIP occupant', () => {
    const board = makeBoard();
    ...
    applyBoardAutomation({
      ...
      companionDir: tmpDir,
      boardBaseName: 'board',
      previousWipTitle: 'Tarea B',
      justMovedTitle: null,
      now: new Date(2026, 7, 22, 11, 0),
    });

    expect(readRunningTimerTask(tmpDir)).toBe('Fuera del board');
  });

  it('enforces single WIP and keeps the just-moved task', () => {
    const board = makeBoard();
    board.columns.find((c) => c.title === 'WIP')!.tasks = [
  ...
  it('does nothing to the board when WIP occupant does not change', () => {
    const board = makeBoard();
    board.columns.find((c) => c.title === 'WIP')!.tasks = [{ id: 't-a', title: 'Tarea A' }];
